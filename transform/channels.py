...
def transform(db_context, etl_process_id):
    channels_extract = pandas.read_sql_table(table_name='CHANNELS_EXT', con=db_context.connection, columns=['CHANNEL_ID', 'CHANNEL_DESC', 'CHANNEL_CLASS', 'CHANNEL_CLASS_ID'])
    ...
    if not channels_extract.empty:
        ...
        channels_extract.to_sql(name='CHANNELS_TRA', con=db_context.connection, if_exists='append', index=False)