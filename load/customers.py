...
def load(staging_db_context, core_db_context, etl_process_id):
    ...
    countries_key_pairs = query_utils.get_surrogate_key_and_natural_key_pairs(table_name='COUNTRIES', natural_key_cols=['COUNTRY_ID'], db_context=core_db_context)
    customers['COUNTRY_ID'] = customers['COUNTRY_ID'].apply(lambda x: countries_key_pairs[x])

    query_utils.upsert(table_name='CUSTOMERS', natural_key_cols=['CUST_ID'], dataframe= customers, db_context=core_db_context)
